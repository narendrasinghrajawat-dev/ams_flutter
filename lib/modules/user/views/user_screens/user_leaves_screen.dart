      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            Obx(() {
              final List<ApplyLeaveRequest> all = _userLeavesController.filteredAppliedLeavesList;

              // Group by status (lowercase for safety)
              final approved = all.where((e) => (e.leaveStatus ?? '').toLowerCase() == AppStrings.approvedStatusKey).toList();
              final pending = all.where((e) => (e.leaveStatus ?? '').toLowerCase() == AppStrings.pendingStatusKey).toList();
              final rejected = all.where((e) => (e.leaveStatus ?? '').toLowerCase() == AppStrings.rejectedStatusKey).toList();
...
class _LeaveCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final status = (leave.leaveStatus ?? 'pending').toLowerCase();
    final appliedAt = leave.actionDate ?? ''; // e.g. 2025-12-04T13:13...
    final bool isPending = status == AppStrings.pendingStatusKey;
    ...
    return CommonCardWidget(
      child: Column(
        ...
        children: [
            ...
            children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(
                    color: AppHelper.getLeavesStatusColor(leave.leaveStatus).withOpacity(0.6),
                  ),
                ),
                child: AppTextWidget.verySmall(
                  AppHelper.getLeavesStatusValue(leave.leaveStatus),
          ...
          Row(
            children: [
              Icon(Icons.event_note, size: 16, color: AppThemeColors.iconColor),
              ...
              if (appliedAt.isNotEmpty) ...[
                Icon(Icons.access_time, size: 14, color: AppThemeColors.iconColor),
                const SizedBox(width: 4),
